...
const TablaProdRotorIse = () => {
    ...
    const consultarDatos = useCallback(async (fecha, turno, isPoll = false) => {
        ...
        try {
            // Llamadas endpoints de Rotor Insinkerator
            const pTabla = axios.get(`http://10.13.225.20:3003/api/rotise/produccion-real?fecha=${fecha}&turno=${turno}`);
            const pTotal = axios.get(`http://10.13.225.20:3003/api/rotise/total-dia?fecha=${fecha}`);
            const pGuardado = axios.get(`http://10.13.225.20:3003/api/rotise/reporte-guardado?fecha=${fecha}&turno=${turno}`);
    ...
    const fetchAndSyncData = useCallback(async () => {
        ...
        try {
            const pTabla = axios.get(`http://10.13.225.20:3003/api/rotise/produccion-real?fecha=${selectedDate}&turno=${selectedShift}`);
            const pTotal = axios.get(`http://10.13.225.20:3003/api/rotise/total-dia?fecha=${selectedDate}`);
    ...
    useEffect(() => {
        document.title = "Tabla de Producción Rotor Insinkerator";
        if (selectedShift !== '0') { 
    ...
    const handleGuardar = async () => {
        ...
        try {
            await axios.post('http://10.13.225.20:3003/api/rotise/guardar-reporte', reportData);
    ...
    return (
        <div className="bodyTabla-rotwet">
            <title>Tabla Productividad Rotor ISE</title>
            <header className="header-rotwet">
                <img src={logoNidec} alt="Nidec ACIM Logo" className="logoTabla-rotwet" />
                <h2 className="tituloPrincipal-rotwet">TABLA DE PRODUCTIVIDAD ROTOR INSINKERATOR</h2>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select value={lider} onChange={(e) => setLider(e.target.value)}>
                                            <option value="0" disabled>--Selecciona--</option>
                                            <option value="Rosa Irene Gonzalez">Rosa Irene Gonzalez</option>
                                        </select>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Línea:</td>
                                    <td>
                                        <select value="4" onChange={handleLineChange}>
...
export default TablaProdRotorIse;