...
const TablaProdInsin = () => {
    ...
    const [metaPorHora, setMetaPorHora] = useState(130); 
    
    // Modales
    const [isMetaModalOpen, setIsMetaModalOpen] = useState(false);
    const [tempMetaInput, setTempMetaInput] = useState(130);
    ...
    const consultarDatos = useCallback(async (fecha, turno, isPoll = false) => {
        ...
        try {
            // Llamadas endpoints de Insinkerator
            const pTabla = axios.get(`http://10.13.225.20:3003/api/insi/produccion-real?fecha=${fecha}&turno=${turno}`);
            const pTotal = axios.get(`http://10.13.225.20:3003/api/insi/total-dia?fecha=${fecha}`);
            const pGuardado = axios.get(`http://10.13.225.20:3003/api/insi/reporte-guardado?fecha=${fecha}&turno=${turno}`);
    ...
    const fetchAndSyncData = useCallback(async () => {
        ...
        try {
            const pTabla = axios.get(`http://10.13.225.20:3003/api/insi/produccion-real?fecha=${selectedDate}&turno=${selectedShift}`);
            const pTotal = axios.get(`http://10.13.225.20:3003/api/insi/total-dia?fecha=${selectedDate}`);
    ...
    useEffect(() => {
        document.title = "Tabla de Producción Insinkerator";
        if (selectedShift !== '0') { 
    ...
    const handleGuardar = async () => {
        ...
        try {
            await axios.post('http://10.13.225.20:3003/api/insi/guardar-reporte', reportData);
    ...
    const diaStatusClass = totalDia >= metaDiaAcumulada ? 'status-good-insi' : 'status-bad-insi';
    ...
    return (
        <div className="bodyTabla-insi">
            <title>Tabla Productividad Insinkerator</title>
            <header className="header-insi">
                <img src={logoNidec} alt="Nidec ACIM Logo" className="logoTabla-insi" />
                <h2 className="tituloPrincipal-insi">TABLA DE PRODUCTIVIDAD INSINKERATOR</h2>
                <img src={Zero} alt="Zero Productividad" className="logoZero-insi" />
            </header>

            <div className="top-panel-container-insi">
                <div className="panel-left-insi">
                    <section className="datosGenerales-insi">
                        <table>
                            <tbody>
                                <tr>
                                    <td>Fecha:</td>
                                ...
                                <tr>
                                    <td>Línea:</td>
                                    <td>
                                        <select value="2" onChange={handleLineChange}>
                ...
                <div className="panel-right-insi">
                    <div className="medidores-container-insi">
                        <div className="medidor-insi">
                            <h3>Meta</h3>
                            <div className="gauge-wrapper-insi">
                                <GaugeChart 
                                ...
                                />
                                <div className="gauge-value-text-insi">{totalMetaTurno}</div>
                            </div>
                        </div>
                        <div className="medidor-insi">
                            <h3>Real</h3>
                            <div className="gauge-wrapper-insi">
                                <GaugeChart 
                                    ...
                                    animate={enableAnimation} 
                                />
                                <div className="gauge-value-text-insi" style={{ color: totalReal >= totalMetaTurno ? '#388E3C' : '#333' }}>
                                    {totalReal}
                                </div>
                            </div>
                        </div>
                        <div className="medidor-insi">
                            <h3>Pérdidas</h3>
                            <div className="donut-wrapper-insi">
                                <CircularProgressbar 
                        ...
                        <div className="medidor-insi-card">
                            <div className={`card-total-dia-insi ${diaStatusClass}`}>
                                <div className="total-dia-content-wrapper-insi">
                                    <div className="total-dia-left-insi">
                                        <h4>Total Día</h4>
                                        <p className="total-dia-number-insi">
                                            {totalDia} 
                                            <span style={{fontSize:'0.9rem', fontWeight:'normal', color:'#777'}}>/ {metaDiaAcumulada}</span>
                                        </p>
                                        <div className="delta-container-insi" style={{color: deltaColor}}>
                                            Delta: {deltaSigno}{delta}
                                        </div>
                                    </div>
                                    <div className="total-dia-right-breakdown-insi">
                                        <div className="breakdown-item-insi">
                                ...
                                <div className="progress-bar-container-insi">
                                    <div className="progress-bar-fill-insi" style={{ width: `${percentDia}%` }}></div>
                                </div>
                            </div>
                        </div>

                        <div className="medidor-insi-image">
                            <img 
                                src={diaStatusClass === 'status-good-insi' ? ZeroBien : ZeroMal} 
                                alt="Estado de Producción" 
                                className="indicador-imagen-insi"
                            />
                        </div>
            ...
            <section className="contenedorTabla-insi">
                <table className="tablaProduccion-insi">
                    ...
                    <tbody>
                        {hours.map((timeSlot, index) => {
                            ...
                            if (!isNaN(realNum) && realValDisplay !== '##' && realNum >= meta) {
                                cellClass = "celda-cumplida-insi";
                            } else if (isPastHour) {
                                cellClass = "celda-no-cumplida-insi";
                            }
                                ...
                                <tr key={index}>
                                    ...
                                    <td>
                                        <input
                                            type="text"
                                            readOnly 
                                            className="input-loss-clickable-insi"
            ...
            <div className="div-btn-guardar-insi">
                <button className="btnCambiarMeta-insi" onClick={openMetaModal}>Ajustar Meta</button>
                <button className="btnGuardarTabla-insi" onClick={handleGuardar}>Guardar</button>
            </div>

            {isMetaModalOpen && (
                <div className="modal-overlay-insi">
                    <div className="modal-content-insi">
                        <h3>Definir Meta por Hora</h3>
                        <p>Ingresa la nueva cantidad:</p>
                        <input type="number" className="input-meta-modal-insi" value={tempMetaInput} onChange={(e) => setTempMetaInput(e.target.value)} />
                        <div className="modal-actions-insi">
                            <button className="btn-modal-cancel-insi" onClick={closeMetaModal}>Cancelar</button>
                            <button className="btn-modal-confirm-insi" onClick={confirmNewMeta}>Aceptar</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isLossModalOpen && (
                <div className="modal-overlay-insi">
                    <div className="modal-content-large-insi">
                        <h3>Registro de Eventos: {currentLossSlot}</h3>
                        <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
                            <table className="loss-list-table-insi">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    {tempLossList.map((item, idx) => (
                                        <tr key={idx}>
                                            ...
                                            <td><button className="btn-delete-loss-insi" onClick={() => removeLossItem(idx)}>X</button></td>
                                        </tr>
                                    ))}
                        ...
                        <div className="total-display-insi">
                            Total Pérdidas: {tempLossList.reduce((sum, item) => sum + (parseInt(item.minutos)||0), 0)} min
                        </div>
                        
                        <div className="add-loss-form-insi">
                            <div className="form-group-insi">
                                <label>Minutos:</label>
                            ...
                            <div className="form-group-insi" style={{flex:1}}>
                            ...
                            <button className="btn-add-loss-insi" onClick={addLossItem}>Agregar</button>
                        </div>

                        <div className="modal-actions-insi" style={{marginTop: '10px'}}>
                            <button className="btn-modal-cancel-insi" onClick={closeLossModal}>Cancelar</button>
                            <button className="btn-modal-confirm-insi" onClick={saveLossesFromModal}>Aceptar</button>
                        </div>
                    </div>
                </div>
            )}
